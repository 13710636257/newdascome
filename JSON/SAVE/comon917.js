...
$(document).ready(function () {
    ...
    if (data) {
        var username = JSON.parse(data)['user'];
    ...
    //只要登录页面的都 做一个标志
    //bug: 存在一个，在注册页面后再登录 那么就会 history.back() 回到注册页面了
    // var href = location.href;
    // var islogin = new RegExp('login.html', 'g').test(href);
    // if (!islogin) {
    //     sessionStorage.setItem('io', 'bG92ZQ==');//love
    // } else {
    //     sessionStorage.setItem('io', 'iii');//iii
    // }
    ...
    $win.bind({
        ...
        storage: function (event) {
            ...
            //获取登录成功后的数据
            //没有登录 value 为null
            var session = Storage.get(Message.kn);
            if (session === null) return false;
            // Storage.set(Message.kn+'..call', Storage.fy(session));
            // localStorage.setItem(Message.kn+'.call' ,Storage.fy(session));
            // Storage.set(Message.kn+'...call', Storage.fy(session));


            // var co = cookie.getCookie(Message.kn);
            // if(co ===null){
            //     cookie.setCookie(Message.kn,Storage.fy(Storage.get(Message.kn)))
            // }else{
            //     var da = cookie.getCookie(Message.kn)
            //     console.log(da)
            //     Storage.set(Message.kn+'.call',da);
            //     //cookie.removeCookie(Message.kn)
            // }
